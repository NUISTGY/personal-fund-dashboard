...
- 投资记录新增、卖出、删除操作会实时写入 `./data/investment-records.json`，同一服务下不同浏览器打开页面时会加载同一份记录。
...
- 投资记录保存于 `./data/investment-records.json`，该文件由本地服务自动创建且不纳入 Git 版本控制；统计结果按记录类型、成交净值、持有份额与当前基金净值计算，浏览器本地存储仅作为接口异常时的临时备份。
